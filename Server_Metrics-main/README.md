...
Grafana provisions the **Server Metrics // Nightwatch** dashboard automatically
from `infra/grafana/provisioning/dashboards`. It uses a dark, low-noise visual
system with restrained slate, teal, and violet accents:

* CPU, memory pressure, memory used, and active host overview cards
* Resource and memory time-series panels with a five-second refresh
* Live Loki event stream

Start the infrastructure stack with `docker compose` from `infra/`. Open
`http://localhost:9000` and select the **Operations / Server Metrics // Nightwatch**
dashboard if Grafana does not open it automatically.
